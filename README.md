...
| `tasks-management` | Reference for the `.tasks/TASKS.md` contract — the markdown task format, the read/write/complete verbs, and how to surface overdue / due-today / priority items. Reference skill, set up by `/tasks-start`. |
| `tasks-memory` | The two-tier workplace-memory model (`.tasks/CLAUDE.md` hot cache + `.tasks/memory/` deep store) that lets Claude decode shorthand, nicknames, acronyms, and project codenames like a colleague. Reference skill. |
| `tasks-remove` | Decommission the tasks system and flatten it back into the repo — merges working memory into the root `CLAUDE.md`, moves deep memory into a repo-level `memory/`, preserves open tasks, then deletes `.tasks/`. The inverse of `/tasks-start`. |
| `tasks-start` | Stand up a self-contained `.tasks/` task + workplace-memory system (TASKS.md, working memory, deep memory, and a SHAUGHV-branded board/list/memory `dashboard.html` with a vintage-cream / brutalist-dark theme toggle), then bootstrap memory from your real task list and connected tools. Adapted and expanded from the Anthropic Productivity plugin. |
...
| `/shaughv-code:tasks-start` | Initialize the self-contained `.tasks/` task + workplace-memory system in the current repo and open the SHAUGHV dashboard. |
| `/shaughv-code:tasks-update` | Sync tasks, triage overdue/stale items, and fill memory gaps (`--comprehensive` for a deep scan of chat/email/calendar/docs). |
| `/shaughv-code:tasks-remove` | Tear down the `.tasks/` system, flattening its memory back into the repo's root `CLAUDE.md` and `memory/`. |
