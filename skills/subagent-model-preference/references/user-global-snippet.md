...
> It applies to every project. Set 2026-07-24 per Emmett.
...
Only the **Opus** and **Sonnet** classes are in scope. Never a weaker model (no Haiku/older), and do
not auto-substitute the distinct **Fable or Mythos classes**. Never below `xhigh` effort. Which model
+ which of its two efforts is the orchestrating/spawning agent's call per situation — just honor
each model's preferred default and the floor. In `Workflow` scripts pass model + effort explicitly per agent
...
1. Find the current **Opus-class** and **Sonnet-class** models (same class lineage as today). Ignore
   Haiku, and ignore any other class (e.g. Fable or Mythos) unless this preference is explicitly updated to include it.
