...
Release 0.35.0 adds the OAuth-gated Pipedream MCP endpoint to every plugin
surface and keeps the Git workflow strict by default while allowing clear owner
approval to simplify the delivery route without waiving validation.

## Project status

- **Current release:** 0.35.0 (14 July 2026).
- **Default branch:** `main`.
...
- **Current state:** Claude, Codex, and marketplace manifests are version-aligned;
  the generated Codex package contains 29 skills and 461 files.
...
2. Bump the same version in the Claude plugin manifest, Claude marketplace
   entry, and Codex plugin manifest.
3. Update `CHANGELOG.md` and `HUMAN_CHANGELOG.md` in lockstep.
4. Regenerate with `pwsh ./build-codex-plugin.ps1`.
5. Validate JSON, skill metadata, scripts, the generated package, secrets, and
   any repository-applicable local checks.
6. Commit and deliver through the preferred PR workflow, or use an explicitly
   owner-approved direct default-branch route without skipping quality checks.
7. Verify the exact pushed commit and its GitHub Actions run.
...
This tree includes the entire project workspace except Git's internal
`.git/` object database. The generated Codex package is included in full.
